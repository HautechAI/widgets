...
import type { Image } from '@hautechai/client';
...
const useLogic = (props: Props) => {
    ...
    const canRedo = useMemo(() => (stack.metadata.undone?.length ?? 0) > 0 && !loading, [loading, stack.metadata]);
    const canUndo = useMemo(() => stack.operations.length > 2 && !loading, [loading, stack.operations]);
    const canUpscale = useMemo(
        () => stack.operations.every((operation) => operation.type !== 'upscale.v1') && !loading,
        [loading, stack],
    );
    ...
    const onRetouch = useCallback(
        async (category: 'bottom' | 'dress' | 'top') => {
            ...
            await runAction(async () => {
                ...
                describeOperation = await sdk.operations.wait({ id: describeOperation.id });
                if (!describeOperation.output?.text) throw new Error('Failed to describe product');
                ...
                await stacksAPI.addOperations({ id: stack.id, operationIds: [operation.id] });
            });
        },
    ...
    const onUpscale = useCallback(async () => {
        ...
        await runAction(async () => {
            const operation = await sdk.operations.create.upscale.v1({ input: { imageId } });
            await stacksAPI.addOperations({ id: stack.id, operationIds: [operation.id] });
            await stacksAPI.update({ id: stack.id, metadata: { undone: [] } });
        });
    }, [canUpscale, imageId, runAction, stack.id, stacksAPI.addOperations, stacksAPI.update]);
    ...
    const running = useMemo(
        () => loading || stack.operations.some((operation) => operation.status === 'pending'),
    ...
    const undo = useCallback(async () => {
        ...
        await runAction(async () => {
            ...
            await stacksAPI.removeOperation({ id: stack.id, operationId: operation.id });
            await stacksAPI.update({
                id: stack.id,
                metadata: { undone: [operation.id, ...(stack.metadata.undone ?? [])] },
            });
        });
    }, [canUndo, runAction, stack.id, stacksAPI.removeOperation, stacksAPI.update]);
