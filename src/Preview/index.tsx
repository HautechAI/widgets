...
import SingleImageUploader, { UploadedImage } from './components/SingleImageUpload';
import { ColumnContainer, Container, LinkButton, Row, Section, SectionLabel, SectionTitle, Status } from './styles';
...
export const Preview = () => {
    ...
    const handleProductImageSelection = async (selection: UploadedImage) => {
        ...
        const productImage = await sdk.images.createFromFile(selection.file);

        const productInfoResponse = await sdk.operations.wait(
            await sdk.operations.run.gpt.v1({
                input: {
                    prompt: 'Detect main product on the image and create short description of a model wearing this product in the studio. Return the description as json { product, apparelType, modelDescription }',
    ...
    const handlePoseImageSelection = async (selection: UploadedImage) => {
        ...
        const poseImage = await sdk.images.createFromFile(selection.file);

        const poseResponse = await sdk.operations.wait(
            await sdk.operations.run.poseEstimation.v1({
                input: {
    ...
    return (
        <Container>
            <ColumnContainer>
                ...
                <Section>
                    <SectionTitle>Step 2: Prepare collection for generated images</SectionTitle>
                    <Row>
                        Enter existing
                        <input
                            type="text"
                ...
                <Section>
                    <SectionTitle>
                        Step 3: Prepare product image {form.isProcessingProductImage && <Status>Loading</Status>}
                    </SectionTitle>
                    <SingleImageUploader
                ...
                <Section>
                    ...
                    <ToggleButtonGroup
                        ...
                        size="small"
                        exclusive
                        onChange={(_, value) => setForm((f) => ({ ...f, model: value }))}
                    >
            ...
            <ColumnContainer>
                ...
                <Section>
                    ...
                    <button
                        disabled={
                            !form.productImageId || !form.collectionId || (!form.poseId && form.model === 'naomi')
                        }
                        onClick={() => {
                            widgetMethods?.start?.();
                        }}
                ...
                <Section>
                    ...
                    <Row>
                        {selectedImages.map((url) => (
                            <img key={url} src={url} style={{ maxWidth: '100px', maxHeight: '100px' }} />
                        ))}
                    </Row>
                </Section>
            </ColumnContainer>
